...
public class SoakRequest {
	...
	public static Response sendRequest(Horse horse, Bot bot) throws ApiException {
		try {
			...
			if(!horse.info.canSoak)
				throw new Exception("Cannot soak (drink) yet!");
			
			// we gotta get some random shit ^^ (even more than normal)
			String form = horse.info.html.split("method=\"post\" id=\"form-do-drink\">")[1].split("</form>")[0];
			
			
			String theShitName = form.split("name=\"")[1].split("\"")[0];
			String theShitVal = form.split("<input value=\"")[1].split("\"")[0];
			String scndShit = form.split("<input name=\"")[1].split("\"")[0].split("form-do-drink")[1];
			
			String[] split2 = form.split("<input name=\"form-do-drink");
			...
			JsonObject json = GenericPostRequest.doApiCall("elevage/chevaux/doDrink", new String[] {theShitName, scndShit, pxName, pyName}, new String[] {theShitVal, ""+horse.id, "" + new Random().nextInt(50), "" + new Random().nextInt(50)}, bot);
